...
Build SEAHOWL with ``SEAHOWL_ENABLE_PYTHON`` as ``ON``. You can then add the build directory to your ``PYTHONPATH`` so that the Python executable used to build the bindings can ``import seahowl`` from anywhere.
