...
// Bug in bladed doc ? 2 times in the doc a in or out . 2 different significations {49, "out", 'I', "Message length OR -M0", "-"},
...
void seahowl::servo::DisconController::Init(std::string infile, std::string outname) {
    ...
    for(int i=0; i<150;++i ) {
        std::cout << i << "  "<< discon::ArrayInfo[i].index << "  " << discon::ArrayInfo[i].description << "" << std::endl;
        if(discon::ArrayInfo[i].index  != i) {
            std::cerr << "ERROR IN ARRAY index " << i << std::endl;
        }
    }*/
