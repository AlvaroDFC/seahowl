...
std::shared_ptr<seahowl::elasto::BladeElasto> get_blade_elasto_from_db(const BladeTurbineDb& blade_db,
    ...
    blade_elasto->azimuth_override = blade_db.azimuth_override.has_value()
                                         ? blade_db.azimuth_override.value() * PI / 180.0
                                         : -1.0;
...
std::shared_ptr<seahowl::fluid::TurbineFluid> get_turbine_aero_from_db(const TurbineDb& turbine_db) {
    ...
    if (turbine_db.aero.solver == "aerodyn") {
        ...
        turbine_aero =
            std::make_shared<seahowl::aero::TurbineAeroDyn>(file_aerodyn_path, turbine_db.rotor.vertical_axis, turbine_db.rotor.MHK);
#endif
    } else {
